...
public class Test {
	...
	public static List<FQLlikes> getLikesForUserPhotos(FacebookClient facebookClient, String uid) {
		if (uid == null || "me".equals(uid))
			return new LinkedList<FQLlikes>();
	...
	public static List<FQLPhoto> getAllUserPhotos(FacebookClient facebookClient, String uid) {
		if (uid == null || "me".equals(uid))
			return new LinkedList<FQLPhoto>();
	...
	public static List<FQLStatus> getUserStatuses(FacebookClient facebookClient, String uid) {
		if (uid == null || "me".equals(uid))
			return new LinkedList<FQLStatus>();
	...
	public static List<FQLFriend> getFriends(FacebookClient facebookClient) {
		...
		for (FQLFriend friend : friends) {
			friend.name = Normalizer.normalize((friend.name), Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "");
		}
		return friends;
	}
	...
	public static void getPopularPhotos(Vector<String> popularPhotoId, Vector<String> popularPhotoImage, Vector<Integer> popularPhotoCount, String userId, FacebookClient facebookClient) {
		if (userId == null || "".equals(userId))
			userId = "me";

		// FacebookClient publicOnlyFacebookClient = new
		// DefaultFacebookClient();
		// User user = facebookClient.fetchObject("me", User.class);
		// Page page = facebookClient.fetchObject("cocacola", Page.class);

		// System.out.println("User name: " + user.getName() + " and hometown: "
		// + user.getHometownName());
		...
		// Connection<Photo> myPhotos =
		// facebookClient.fetchConnection("me/photos", Photo.class,
		...
		// myFeed.getData().get(0));

		// Connections support paging and are iterable

		for (List<Photo> photoList : myPhotos)
			for (Photo photo : photoList) {
				// System.out.println("Photo ("+ index +") id: " + photo.getId()
				// + " likes: "+ (photo.getLikes().size() < 25 ?
				// photo.getLikes().size() : numberOfLikes(facebookClient,
				// photo.getId())) +" img:"+photo.getPicture());
